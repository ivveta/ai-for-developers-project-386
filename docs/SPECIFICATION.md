...
> **Статус реализации.** Фронтенд выполнен по [FRONTEND-PLAN](./FRONTEND-PLAN.md): все экраны §7.1
> реализованы, критерии интерфейса F1–F5 (§11) покрыты тестами. Бэкенд реализован по
> [BACKEND-PLAN](./BACKEND-PLAN.md): API §8 на Fastify + PostgreSQL, критерии приёмки A–E (§11)
> покрыты юнит- и интеграционными тестами; в проде бэкенд раздаёт собранный фронтенд — один
> сервис (STRUCTURE-PLAN, шаг 5). Запуск: `make db` и `make dev` (см. README).
...
|---|---|---|---|
| Р1 | Длительность типа события | Произвольная: целое число от 1 до 540 минут, задаёт владелец при создании | Согласовано с заказчиком; 540 минут — длина рабочего дня, поэтому у любого типа в дне есть хотя бы один слот |
...
| Р11 | Владелец календаря | Один заранее заданный профиль | Исходное описание + скриншоты `02` и `03` |
...
| `durationMinutes` | int | да | Длительность встречи в минутах: целое число от 1 до 540 (Р1) |
...
| И2 | `EventType.durationMinutes` — целое число от 1 до 540 (минут) |
...
где 540 — длина рабочего дня в минутах (09:00–18:00). В сетку входят только слоты, целиком
помещающиеся в рабочий день.
...
| 100 мин | 5 | 09:00 – 10:40 | 15:40 – 17:20 |

Длительность произвольная (Р1), поэтому 540 может не делиться на `D` нацело: неполный «хвостовой»
слот у границы 18:00 не генерируется (при `D = 100` интервал 17:20–18:00 для этого типа
недоступен). Сетки разных типов не обязаны совпадать: занятость определяется пересечением
интервалов (§5.3) и от совпадения границ не зависит.
...
| Длительность типа не делит 540 нацело | Неполный слот у конца рабочего дня не генерируется (§5.1) |
| У дня окна не осталось доступных слотов (все в прошлом) | День присутствует в выдаче с пустым массивом слотов и `freeCount = 0`, не ошибка |
...
**Результат:** выбраны тип события, дата и время; система показывает форму записи.
...
1. На той же странице `/book/{eventTypeId}` гость видит сводку: тип события, дату, время,
   длительность.
...
### С4. Владелец открывает админ-панель

1. Владелец открывает `/admin`.
2. Система показывает заголовок «Админ-панель» и две карточки-ссылки: «Предстоящие встречи»
   и «Новый тип события».
3. Владелец переходит на нужный раздел.

**Результат:** владелец попадает в нужную часть админки из одного хаба.

### С5. Владелец создаёт тип события

1. Владелец открывает `/admin/event-types` и нажимает «Создать тип события».
2. Заполняет `id`, название, описание и задаёт длительность в минутах — целое от 1 до 540 (Р1).
...
### С6. Владелец просматривает предстоящие встречи
...
sequenceDiagram
    ...
    UI->>API: GET /api/event-types/{id}/slots
    API->>DB: Брони, пересекающие окно записи
    DB-->>API: Список броней
    API-->>UI: Слоты 14 дней со статусами free / busy
...
| `/book/{eventTypeId}` | Выбор даты и слота, форма записи | Гость |
| `/book/{eventTypeId}/success` | Успешное бронирование | Гость |
| `/admin` | Админ-панель: хаб с переходами | Владелец |
...
flowchart LR
    ...
    Slots --> Success["/book/{id}/success"]
    Slots -. "слот занят" .-> Slots
    Slots --> Catalog
    Success --> Catalog

    Home --> AdminHub["/admin"]
    AdminHub --> Admin["/admin/bookings"]
    AdminHub --> Types["/admin/event-types"]
...
(на `/book`) и «Админка» (на `/admin`). Активный раздел визуально выделен.
...
- Сетка карточек типов событий. В карточке: название, описание, бейдж длительности (число
  и «мин», формат §7.12). Вся карточка кликабельна и ведёт на `/book/{eventTypeId}`.
...
- Кнопки «Назад» (в каталог) и «Продолжить». «Продолжить» неактивна, пока слот не выбран;
  после нажатия на той же странице показывается форма записи (§7.6), а выбранный слот
  фиксируется в query-параметре `startAt`.

### 7.6. Форма и подтверждение — шаг экрана `/book/{eventTypeId}`

Отдельного маршрута нет: форма показывается на странице `/book/{eventTypeId}` после выбора слота
и нажатия «Продолжить». Выбранный слот передаётся query-параметром `startAt` — меткой времени
ISO 8601 со смещением, как в API (в URL — percent-encoded), например
`/book/meeting-15?startAt=2026-04-01T10:00:00%2B03:00`. При наличии параметра страница открывается
сразу на шаге формы, поэтому перезагрузка не теряет контекст. Некорректное значение параметра
игнорируется: показывается шаг выбора слота.

- Сводка: тип события, длительность, дата, время.
- Поля: «Имя» (обязательное), «Email» (обязательное), «Заметка» (необязательное, многострочное).
- Кнопки «Назад» (к выбору слота, параметр `startAt` снимается) и «Записаться».
...
### 7.8. Админ-панель `/admin`

Хаб админской части. На экране заголовок «Админ-панель» и две карточки-ссылки:

- «Предстоящие встречи» → `/admin/bookings`
- «Новый тип события» → `/admin/event-types/new`

Каждая карточка содержит название и краткое описание. Вся карточка кликабельна.

### 7.9. Админка: типы событий
...
| Длительность | Числовое поле: целое число минут от 1 до 540 (решение Р1) |

Кнопки «Отмена» и «Создать».

### 7.10. Админка: предстоящие встречи `/admin/bookings`
...
| Слот занят | `/book/{id}`, шаг формы | «Этот слот только что заняли. Выберите другое время» и возврат к выбору слота с обновлёнными данными |
| Ошибка запроса | Все экраны | Сообщение об ошибке и возможность повторить |

### 7.12. Форматы отображения
...
| `GET` | `/api/event-types/{id}/slots` | Слоты типа события на всё окно записи (14 дней) |
...
### 8.4. ~~`GET /api/event-types/{id}`~~ — удалён

Эндпоинт убран: ни одному сценарию (§6) не нужен просмотр одного типа события. Карточка типа на
странице бронирования (§7.5) строится по данным каталога (§8.2), а экран «Тип события не найден»
(§7.11) опирается на `404` от §8.5. Нумерация последующих разделов сохранена.

### 8.5. `GET /api/event-types/{id}/slots`

Слоты указанного типа события на всё окно записи: 14 дней, начиная с текущей даты (§5.2).
Параметров нет — одним запросом клиент получает все данные календаря (§7.5).

Ответ содержит ровно 14 элементов `days` — по одному на каждый день окна, первый — текущая дата.
Дней вне окна в ответе нет. Из выдачи исключены слоты с началом в прошлом; если у дня не осталось
доступных слотов (например, текущий момент позже 18:00), день присутствует с пустым массивом
`slots` и `freeCount = 0`.

**200 OK** — фрагмент ответа для `meeting-15`, текущий момент 31 марта 2026, 09:00, первые три
  ...
  "data": {
    ...
    "days": [
      ...
      },
      {
        ...
        "freeCount": 36,
        "slots": [
          { "start": "2026-04-01T09:00:00+03:00", "end": "2026-04-01T09:15:00+03:00", "status": "free" }
        ]
      }
    ]
  }
...
Поле `freeCount` дня используется для счётчика «33 св.» под датой в календаре.
...
Список предстоящих бронирований — встреч, которые ещё не начались (§7.10). Параметров нет:
прошедшие встречи не возвращаются никогда.
...
| `durationMinutes` | Обязательно | «Укажите длительность» |
| `durationMinutes` | Целое число от 1 до 540 | «Длительность — целое число от 1 до 540 минут» |
...
| A3 | **Given** любая система **When** владелец создаёт тип с `durationMinutes=541` **Then** `400 validation_error` с указанием поля `durationMinutes`, тип не создан |
| A4 | **Given** любая система **When** владелец создаёт тип с `id="Meeting 15"` **Then** `400 validation_error`, тип не создан |
| A5 | **Given** форма создания типа события **When** владелец её открывает **Then** длительность задаётся числом минут, допустимы целые значения от 1 до 540 |
| A6 | **Given** пустая система **When** владелец создаёт тип с `durationMinutes=45` **Then** возвращается `201`, тип виден в `GET /api/event-types` |

### Слоты

| № | Given / When / Then |
|---|---|
| B1 | **Given** тип 15 минут, в окне нет броней **When** запрошены слоты **Then** у будущего дня выдачи ровно 36 слотов, первый 09:00–09:15, последний 17:45–18:00, все `free`, `freeCount = 36` |
| B2 | **Given** тип 30 минут, те же условия **When** запрошены слоты **Then** у будущего дня выдачи ровно 18 слотов, первый 09:00–09:30, последний 17:30–18:00, все `free` |
| B3 | **Given** текущее время 11:20, тип 15 минут **When** запрошены слоты **Then** у сегодняшнего дня выдачи слоты до 11:30 отсутствуют, первый слот 11:30–11:45 |
| B4 | **Given** любой тип **When** запрошены слоты **Then** ответ содержит ровно 14 дней: первый — текущая дата, последний — 14-й день; дат вне окна в ответе нет |
| B5 | **Given** в окне нет броней **When** запрошены слоты **Then** у 14-го (последнего) дня выдачи слоты присутствуют |
| B6 | **Given** тип 100 минут, в окне нет броней **When** запрошены слоты **Then** у будущего дня выдачи ровно 5 слотов, первый 09:00–10:40, последний 15:40–17:20; слот, выходящий за 18:00, не генерируется |

### Занятость

| № | Given / When / Then |
|---|---|
| C1 | **Given** бронь типа 30 минут на 09:00–09:30 **When** запрошены слоты типа 15 минут **Then** в выдаче этого дня слоты 09:00–09:15 и 09:15–09:30 имеют статус `busy`, слот 09:30–09:45 — `free` |
| C2 | **Given** бронь типа 15 минут на 09:15–09:30 **When** запрошены слоты типа 30 минут **Then** в выдаче этого дня слот 09:00–09:30 имеет статус `busy`, слот 09:30–10:00 — `free` |
...
| № | Given / When / Then |
|---|---|
| E1 | **Given** владелец открывает `/admin` **When** страница загружена **Then** видны две карточки-ссылки: «Предстоящие встречи» (→ `/admin/bookings`) и «Новый тип события» (→ `/admin/event-types/new`) |
| E2 | **Given** брони разных типов событий **When** владелец открывает `/admin/bookings` **Then** все они в одном списке, отсортированы по времени начала по возрастанию |
| E3 | **Given** прошедшая и будущая брони **When** владелец открывает `/admin/bookings` **Then** показана только будущая |
| E4 | **Given** броней нет **When** владелец открывает `/admin/bookings` **Then** показано пустое состояние, не ошибка |
