...
|---|---|---|
| Корневой `Dockerfile` | ❌ | Есть только `e2e/Dockerfile.backend` (для e2e-стенда). Проверка собирает образ по корневому файлу — его нет |
| Автостарт в контейнере | ❌ | `CMD` есть, но при недоступном PostgreSQL `server.ts` делает `process.exit(1)`. Автопроверка запускает контейнер без БД → приложение упадёт |
...
- [ ] `docker run -e PORT=8080 -p 8080:8080 calendar` без БД → сервер отвечает, SPA открывается
      (контейнер не падает)
- [ ] Полный прогон с БД (`make db` + `DATABASE_URL`) → создание брони через UI работает
