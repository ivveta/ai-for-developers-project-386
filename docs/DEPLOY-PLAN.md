...
- [ ] Создать бесплатный Render Postgres → взять External Database URL
- [ ] Создать Web Service из Docker-образа репозитория, env: `DATABASE_URL`;
      `PORT` платформа задаёт сама
- [ ] Дождаться деплоя, проверить публичную ссылку: открыть SPA, забронировать слот
...
- [ ] Добавить раздел «Демо» с публичным URL в `README.md`
