...
- Сетка карточек типов событий. В карточке: название, описание, бейдж длительности (число
  и «мин», формат §7.11). Вся карточка кликабельна и ведёт на `/book/{eventTypeId}`.
...
(§7.10) опирается на `404` от §8.5. Нумерация последующих разделов сохранена.
...
Список предстоящих бронирований — встреч, которые ещё не начались (§7.9). Параметров нет:
