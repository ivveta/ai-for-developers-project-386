...
7. **Фронт**: при `400` подсвечивает поля из `error.details`; при `409 slot_taken` — сообщение
   ...
   «Тип события не найден» (§7.10). Счётчики «N св.» в календаре — запрос `slots?date=` на каждую
   видимую дату окна (≤14 запросов, для учебного объёма достаточно; эндпоинт спекой зафиксирован
   только per-date).
